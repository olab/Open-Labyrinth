		...
		$.each($(editor.getDoc()).find('.r_entity'), function(index, value) { 
			...
			if(countWords($(value).text())<6){
				...
				$(this).unbind('mouseover').mouseover(function(e) {								
					...
					$(this).append('<span id="typeof'+index+'" style="display:none" class="tooltip"><img id="arrow'+index+'" class="arrow"/>'+prepareTooltipContent($(this),0)+'</span>');
 ...
 // get selected schemas
	...
	$.getJSON(baseURL + "vocabulary/inline/api/?query=types", function(data) {
		...
		a.on("contextmenu",function(event){
            ...
            var context_extras = main_menu;
            var t;
            if (event.preventDefault(), i = a.settings.contextmenu || "link image inserttable | cell row column deletetable", t)
                t.show();
            else{
            var o = [];
            ...
            for (var aa = 0; aa < o.length; aa++)"|" == o[aa].text && (0 === aa || aa == o.length - 1) && o.splice(aa, 1);
                o.push(context_extras[0]);
