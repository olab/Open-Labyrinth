...
/*	var data = encodeURIComponent(txt);
	...
	$.each(dataReceived['Resources'], function(key, val) {
        ...
        entity["type"] = val["semanticType"];
        entity["label"] = val["label"];
        entity["start"] = val["from"];
        entity["end"]= val["to"];
        entity["uri"] = val["uri"];
        entity["exact"] = val["text"];
...
function enrichText(entities, editor) {
	...
	$.each(entities, function(key, val) {
		...
		var selectedContent = oldText.substring(start,end);// val['exact'];
		if (notOverwrite.indexOf(selectedContent.toLowerCase()) == -1) {
			var subjectURI = '';
			if (val['uri']) {
				subjectURI = val['uri'];
			}
