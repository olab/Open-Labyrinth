                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input type="text" name="counter<?php echo $i; ?>" id="counter<?php echo $i; ?>"
                                   value=""/>&nbsp;type +, - or = an integer - e.g. '+1' or '=32'
                        </div>
                    </div>


                    <a class="btn btn-primary"
                       href="<?php echo URL::base() . 'chatManager/removeAddChatQuestion/' . $templateData['map']->id . '/' . $templateData['question_count']; ?>">Remove</a>

                </fieldset>
            <?php } ?>
        <?php } ?>

        <a class="btn btn-primary" href="<?php if (isset($templateData['question_count'])) {
            echo URL::base() . 'chatManager/addChatQuestion/' . $templateData['map']->id . '/' . ($templateData['question_count'] + 1);
        } else {
            echo URL::base() . 'chatManager/addChatQuestion/' . $templateData['map']->id . '/3';
        }?>">Add new</a>
