            ...
            <td valign="top" bgcolor="#bbbbcb">
                ...
                <table width="100%" cellpadding="6">
                    <tr bgcolor="#ffffff"><td align="left">
                            <form id="form1" name="form1" method="post" action="<?php echo URL::base().'labyrinthManager/caseWizard/4/updateCounter/'.$templateData['map']->id.'/'.$templateData['counter']->id; ?>">
                            ...
                            <?php if(isset($templateData['rules']) and count($templateData['rules']) > 0) { ?>
                                <?php foreach($templateData['rules'] as $rule) { ?>
                                    <p><img src="<?php echo URL::base(); ?>images/rule.gif" align="absmiddle" alt="rule"> rule: if '<?php echo $templateData['counter']->name; ?>' is <?php echo $rule->relation->title; ?> <?php echo $rule->value; ?> then go to node <?php echo $rule->redirect_node_id; ?> ('<?php echo $rule->redirect_node->title; ?>') - <a href="<?php echo URL::base().'labyrinthManager/caseWizard/4/deleteRule/'.$templateData['map']->id.'/'.$templateData['counter']->id.'/'.$rule->id.'/'.$rule->redirect_node_id; ?>">delete</a></p>
                                <?php } ?>
                            <?php } ?>
                            <hr>
                            <form id="form2" name="form1" method="post" action="<?php echo URL::base().'labyrinthManager/caseWizard/4/addRule/'.$templateData['map']->id.'/'.$templateData['counter']->id; ?>">
