...
<div class="page-header">
    <div class="pull-right"><?php
        ...
        <a class="btn btn-primary" href="<?php echo URL::base().'dforumManager/addForum'; ?>"><i class="icon-plus-sign"></i> <?php echo __('Add new forum'); ?></a><?php
        } ?>
    </div>
    <h1><?php echo __('Forums'); ?></h1>
</div>
...
<table id="mainForum" class="table table-striped table-bordered">
    ...
    if(isset($templateData['forums']) and count($templateData['forums']) > 0) {
        ...
        foreach($templateData['forums'] as $forum) { ?>
            <tr>
                <td> <?php if(count($forum['topics']) > 0) { ?> <a href="javascript:void(0);" id = "read" class="showMoreTopics" style="text-decoration: none" attr="<?php echo $forum['id']; ?>">&nbsp&nbsp&nbsp&nbsp
                        <i id="icon-<?php echo $forum['id']; ?>" class="icon-chevron-right"></i></a><?php } ?>
                </td>
                <td>
                    <div class="pull-right">
                        <a href="<?php echo URL::base().'dtopicManager/addTopic/'.$forum['id']; ?>" rel="tooltip" title="Add new topic in this forum" class="btn btn-small btn-info">
                            <i class="icon-plus-sign"></i><?php echo __('Add topic'); ?>
                        </a><?php
                        if ($userType == 'superuser' OR $userType == 'Director' OR Auth::instance()->get_user()->id == $forum['author_id']) { ?>
                        <a href="<?php echo URL::base().'dforumManager/editForum/'.$forum['id']; ?>" rel="tooltip" title="Edit this forum" class="btn btn-small btn-info">
                            <i class="icon-edit"></i> <?php echo __('Edit'); ?>
                        </a><?php
                            if ($userType != 'Director') { ?>
                            <a data-toggle="modal" href="javascript:void(0);" data-target="#delete-labyrinth-<?php echo $forum['id'];  ?>" rel="tooltip" title="Delete this forum" class="btn btn-small btn-danger">
                                <i class="icon-trash"></i><?php echo __('Delete Forum'); ?>
                            </a><?php
                            } ?>
                        <div class="modal hide alert alert-block alert-error fade in" id="delete-labyrinth-<?php echo $forum['id'];  ?>">
                            ...
                            <div class="modal-body">
                                ...
                                <p>
                                    <a class="btn btn-danger" href="<?php echo URL::base() . 'dforumManager/deleteForum/' . $forum['id']; ?>"><?php echo __('Delete Forum'); ?></a>
